# Contributing to ep_button_link

Thanks for helping improve `ep_button_link` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_button_link` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_button_link from your local clone
pnpm run plugins i --path ../ep_button_link

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_button_link/` are picked up after a server restart.
...
* Etherpad version, ep_button_link version, browser + OS
* Steps to reproduce
